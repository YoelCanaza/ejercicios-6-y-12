...
class Carta {
public:
    Carta() : valor_(""), palo_(""), mazo_(NULL) {}  // Constructor predeterminado
    Carta(string valor, string palo, Mazo* mazo) : valor_(valor), palo_(palo), mazo_(mazo) {}

    friend ostream& operator<<(ostream& os, const Carta& carta) {
        os << "Carta: " << carta.valor_ << " de " << carta.palo_ << " en el mazo " << carta.mazo_ << mazo_.nombreMazo;
        return os;
    }
